...
def pretty_print_grid( board ):
    for idx in xrange( 3 ):
        print board[3 * idx + 0] + ' | ' + board[3 * idx + 1] + ' | ' + board[3 * idx + 2]

def play_tic_tac_toe():
    playing_board = '_________'
    ai_turn = False
    victor = '_'
    while victor == '_' and playing_board.count( '_' ) != 0:
        pretty_print_grid( playing_board )
        if ai_turn:
            v, playing_board, move = next_move( 'O', playing_board )
            print 'AI played - ' + str( move ) + ' with intention of ' + str( v )
        else:
            print 'Enter your move '
            move = int( raw_input() )
            playing_board = playing_board[:move] + 'X' + playing_board[move + 1:]
        ai_turn = not ai_turn
        victor = eval_board( playing_board )
    pretty_print_grid( playing_board )
